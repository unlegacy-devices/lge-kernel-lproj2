...
/* Fill panel with white pixels right after init sequence.
*  Useful in case when lcdc data not ready.
*  TODO: Enable backlight only after panel contain valid data
*/
#define LGE_PANEL_INIT_QUIRK 1
#define LGE_PANEL_INIT_QUIRK_COLOR 0x00
#endif
...
static void ilitek_qvga_disp_set_rect(int x, int y, int xres, int yres)
{
	if (!disp_initialized)
		return;
...
/* LGE_CHANGE_S: E0 jiwon.seo@lge.com [2011-11-22] : BL control error fix */
	...
	for(y = 0; y < QVGA_HEIGHT; y++) {
		for(x = 0; x < QVGA_WIDTH; x++) {
			EBI2_WRITE16D(DISP_DATA_PORT, LGE_PANEL_INIT_QUIRK_COLOR);
		}
	}
	msleep(30);
#endif
...
static int __init tovis_qvga_init(void)
{
	...
	if (!ret) {
		...
		pinfo->wait_cycle = 0x428000; // 0x908000
